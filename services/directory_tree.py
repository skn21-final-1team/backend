...
from schemas.directory_tree import DirectoryNode, DirectoryTreeResponse, SourceNode
...
class DirectoryTreeService:
    ...
    def get_directory_tree(self, db: Session, notebook_id: int) -> DirectoryTreeResponse:
        """
        notebook_id에 해당하는 모든 디렉토리와 소스를 DB에서 조회한 후
        트리(Tree) 구조로 가공하여 반환합니다.
        ...
        dir_dict: dict[int, DirectoryNode] = {}
        for d in directories:
            node = DirectoryNode.model_validate(d)
        ...
        for s in sources:
            source_node = SourceNode.model_validate(s)
